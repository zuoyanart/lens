...
module.exports = class {
  constructor(layerList = []) {
    this.originalLayerList = layerList.clone();
    this.layerList = layerList.clone();// .layers;
    // create canvas
    this.draw = null;
    this.svgNumber = 0;
    this.layerNumber = 0;
  ...
  async format() {
    this.layerList.style = this.layerList.style || {};
    await this._formatChild(this.layerList, 1);
    // share无法识别关于css中的svg属性，暂时使用此方法做运行测试， 可能要优化puppter
    if (this.svgexportJson.length > 0) {
      const spinner = await tools.loading('字体解析');
      const fontParser = new FontParser(this.fontList);
      fontParser.format();
  ...
  async _formatChild(layerList, level = null) {
    for (let i = 0; i < 1; i++) {
      ...
      this.draw = SVG(document.documentElement).size(layer.frame.width, layer.frame.height).css('background-color', tools.colorParser(layer.backgroundColor));
      this.draw.clear();
      await this._forEachChild(layer, 1);
      // 生成svg
      fs.ensureDirSync(yunjin.WORK_RUNTIME_SNAPSHOT);
      const svg = this.draw.svg();
      // const targetSvgPath = path.resolve(yunjin.WORK_PATH, './target/images/' + layer.do_objectID + '.svg');
      const targetSvgPath = path.resolve(path.join(yunjin.WORK_RUNTIME_SNAPSHOT, '/' + yunjin.config.target.artboardName + ''));

      fs.writeFileSync(targetSvgPath + '.svg', svg.replace(/<br>/ig, '<br/>'));
      // svg导出png
      const range = this._computeLayerRange(layer);
      if (range.left === 0 && range.top === 0 && range.width === layer.frame.width && range.height === layer.frame.height || true) {
  ...
  async _forEachChild(layer, level = 0, drawInstances = this.draw) {
    ...
    if (level === 1) {
      layer.frame.svgX = layer.frame.x;
      layer.frame.svgY = layer.frame.y;
    ...
    for (const item of layers) {
      if (layer._class.toLowerCase().indexOf('group') > -1 || true) {
  ...
  async _formatSvg(layer = {}, draw = {}, layerLength = 0) {
    ...
    if (layer.path) {
      ...
      if (layer.styleResult.backgroundColor && layer.styleResult.backgroundBlendMode) { // && layer.styleResult.backgroundBlendMode
        ...
        // const image =drawInstances.image(imgPath);
  ...
  _isWordArt(layer = {}) {
    ...
    this.fontList.push({
      fontFamily: layer.styleResult.font.fontFamily,
      text: layer.attributedString.string || layer.name
    });
    return true;
  }
  // 给svg元素设置样式
    ...
    if (fillGradient) {
      if (fillGradient.stop) {
        ...
        const pattern = this.draw.pattern(layer.frame.width, layer.frame.height, async(add) => {
          // add.image(imageAsBase64).size(layer.frame.width, layer.frame.height);
